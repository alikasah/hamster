...
CHANNEL_USERNAME = '@sol_extractor_bot'  # Ù‚Ù†Ø§Ø© Ø§Ù„ØªØ­Ù‚Ù‚ Ù…Ù† Ø§Ù„Ø§Ø´ØªØ±Ø§Ùƒ
...
def is_user_subscribed(bot: Bot, user_id: int):
    try:
...
def send_and_remove_codes(chat_id: int, bot: Bot, file_path: str):
    ...
    if len(codes) < 4:
        ...
        return False  # Return False if codes were not sent
    ...
    return True  # Return True if codes were sent successfully
...
def clone(update: Update, context: CallbackContext) -> None:
    ...
    if not is_user_subscribed(context.bot, user_id):
        update.message.reply_text(f'Please subscribe to our channel {CHANNEL_USERNAME} to use this bot.')
        return
    ...
    if send_and_remove_codes(chat_id, context.bot, 'clone.txt'):
...
def train(update: Update, context: CallbackContext) -> None:
    ...
    if send_and_remove_codes(chat_id, context.bot, 'train.txt'):
...
def cube(update: Update, context: CallbackContext) -> None:
    ...
    if send_and_remove_codes(chat_id, context.bot, 'cube.txt'):
...
def bike(update: Update, context: CallbackContext) -> None:
    ...
    if send_and_remove_codes(chat_id, context.bot, 'bike.txt'):
        set_last_usage_time(user_id, 'bike')

def total(update: Update, context: CallbackContext) -> None:
    chat_id = update.message.chat_id

    if not is_user_subscribed(context.bot, update.message.from_user.id):
        update.message.reply_text(f'Please subscribe to our channel {CHANNEL_USERNAME} to use this bot.')
        return
...
To use this bot, you need to subscribe to our channel: {CHANNEL_USERNAME}.
